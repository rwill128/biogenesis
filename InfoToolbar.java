...
public class InfoToolbar extends JToolBar {
	...
	protected JLabel _lEnergy, _lID, _lGeneration, _lAge, _lChildren, _lKills, _lInfected, _lMass, _lReproduceEnergy, _lMaxAge;
	...
	public void setSelectedOrganism(Organism selectedOrganism) {
		...
		_lMaxAge.setText(_selOrganism!=null?_nf.format(_selOrganism.getGeneticCode().getMaxAge()):"0");
		recalculate();
		changeNChildren();
		changeNKills();
		changeNInfected();
	...
	public InfoToolbar(Organism selectedOrganism, MainWindow mainWindow) {
		...
		_lMaxAge = new JLabel(_selOrganism!=null?_nf.format(_selOrganism.getGeneticCode().getMaxAge()):"0",JLabel.CENTER);
